...
export default function FlowEditPage() {
  ...
  async function reorder(indexA, indexB) {
    try {
      const orderA = flow.blocks[indexA].order
      const orderB = flow.blocks[indexB].order
      flow.blocks[indexB].order = orderA
      flow.blocks[indexA].order = orderB

      await updateBlockOrder(flow.blocks[indexB].id, orderA)
      await updateBlockOrder(flow.blocks[indexA].id, orderB)
    } catch { }

    flow.blocks.sort((a, b) => a.order - b.order)
                ...
                <React.Fragment key={index}>
                  <div className="grid grid-cols-[auto_1fr] items-center gap-2 md:hover:[&>.order-button]:opacity-100">
                    <div className="order-button flex flex-col gap-2 mb-10 md:opacity-0 transition-opacity">
                      ...
                      {index < flow.blocks.length - 1 && <Button size="icon" variant="secondary" className="w-8 h-8 cursor-pointer" onClick={() => reorder(index, index+1)}>
