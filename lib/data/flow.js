...
export async function getProgress(userId, blockId) {
  const { data, error } = await supabase
    .from("Progress")
    .select()
    .eq('user_id', userId)
...
export async function createProgress(userId, blockId) {
  const { error } = await supabase
    .from("Progress")
    .insert({
      user_id: userId,
...
export async function deleteProgress(userId, blockId) {
  const { error } = await supabase
    .from("Progress")
    .delete()
    .eq('user_id', userId)
